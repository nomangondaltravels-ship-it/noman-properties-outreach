                        ...
                        <table role="presentation" cellspacing="0" cellpadding="0" width="100%">
                          <tr>
                            ...
                            <td width="50%" style="padding-left:5px">
                              <a href="${cardUrl}" style="display:block;background:#fff8e9;color:#6d4a0f;text-decoration:none;text-align:center;font-weight:800;border:1px solid #ead7a6;border-radius:9px;padding:12px 14px;font-size:14px">View Broker Card</a>
                            </td>
                          </tr>
                        </table>
                      </td>
...
function isSpamRejection(error) {
  return String(error?.message || '').toLowerCase().includes('classified as spam');
}
...
export async function sendContactEmail({ transporter, contact, subject, body }) {
  ...
  try {
    ...
    if (!isSpamRejection(error)) throw error;
