...
export async function addDeleteMarkers(supabase, contacts = []) {
  ...
  if (!error) {
    return [...new Set((data || []).map((row) => row.contact_id).filter(Boolean))];
  }
  ...
  if (campaignError) {
    throw new Error(`${error.message}; marker fallback failed: ${campaignError.message}`);
  }

  return [...new Set((campaign?.results || []).map((row) => row.id).filter(Boolean))];
}
